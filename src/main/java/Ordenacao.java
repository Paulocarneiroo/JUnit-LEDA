...
public class Ordenacao implements Ordenacao_IF{
    ...
    public int partitionQuickRandom(Filme[] filmes, int left, int right){
        Filme pivot = filmes[left + random.nextInt(right - left + 1)];
        int i = left + 1;
        int j = right;
        while (i <= j) {
            while (i <= j && filmes[i].compareTo(pivot) < 0) {
    ...
    public void swapQuickRandom(Filme[] A, int i, int j) {
        Filme temp = A[i];
        A[i] = A[j];
        A[j] = temp;
    }
